...
/**
 * Training algorithm that works by repeatedly changing edge weights to decreasing error
 * It can train objects implementing Train interface
 */
public class BackPropagation {
	...
	public static double runForMinimum(Train trainable, ArrayList<TrainData> trainingData,
			ArrayList<TrainData> testingData, double learningRate, int trainingIterations, int trainingEpoch) {
	...
	public static double runOld(Train trainable, ArrayList<TrainData> trainingData,
			ArrayList<TrainData> testingData, double learningRate, int trainingIterations, int trainingEpoch) {
		LEARNING_RATE = learningRate;
		TRAINING_ITERATIONS = trainingIterations;
		TRAINING_EPOCHS = trainingEpoch;
	...
	public static double runForThreshold(Train trainable, ArrayList<TrainData> trainingData,
			ArrayList<TrainData> testingData, double error_thres, double learningRate, int trainingIterations,
			int trainingEpoch) {
	...
	private static void train(Train trainable, ArrayList<TrainData> trainingData){
	...
	private static void alterEdgeWeights(Train trainable, TrainData trainD){
	...
	private static double testError(Train trainable, ArrayList<TrainData> testingData) {
		// calculate network error across testing data
		double testingError = 0;

		for (TrainData test : testingData) {
			// convert training data to scaled input/output, ie -> [-1, 1]
