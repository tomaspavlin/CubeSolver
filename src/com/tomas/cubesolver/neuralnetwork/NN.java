...
/**
 * Class that defines neural network and it's functions
 */
public class NN implements Train{
	/**
	 * Network squashes all values to inbetween -A and A allows the network to handle massive inputs
	 */
	private static final double A_CONSTANT = 1.716;
	/**
	 * B controls steepness as B smaller, need larger activation to fire neuron
	...
	private Random rng;
	private Solve solveable;
	
	public NN(Solve solvable, int numInput, int numOutput, int numHiddenLayer, int[] hiddenLayerSize){
		this.solveable = solvable;
		
		init(numInput,numOutput, numHiddenLayer, hiddenLayerSize);
	}
	
	public NN(Solve solvable, ArrayList<Integer> layerSizes, 
	...
	@Override
	public Solve getSolvable() {
		return this.solveable;
	}
	public void setSolveable(Solve solveable) {
