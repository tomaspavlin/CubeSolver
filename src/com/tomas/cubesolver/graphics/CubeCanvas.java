...
public class CubeCanvas extends GLCanvas implements GLEventListener{
	...
	private static final long serialVersionUID = 1234567890424242L;
	private Cube cube;
	private int width;
	private int height;
	private FPSAnimator animator;
	private GLU glu;
	private ArrayList<Sticker> allFaces;
	
	public CubeCanvas(GLCapabilities glcapabilities, int width, int height, Cube cube) {
		super(glcapabilities);
		this.addGLEventListener(this);
		this.cube = cube;
		this.allFaces = Drawer.createPuzzleModel(cube);
		this.width = width;
		this.height = height;
	...
	public void display(GLAutoDrawable drawable) {
        ...
        //draw each tile of the cube, one at a time
        for(int i = 0; i < allFaces.size(); i++){
        	Sticker curr = allFaces.get(i);
